export function statement(invoice, plays) {
  const getAmount = (performance) => {
    let amount;
    const type = getTypeByPlayID(performance.playID);
  ...
  for (let perf of invoice.performances) {
    ...
    const thisAmount = getAmount(perf);

    volumeCredits += getCredit(perf);
